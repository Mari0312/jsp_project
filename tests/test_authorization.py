...
def test_refresh(client):
    ...
    resp = client.post(f'/authorization/refresh', headers=headers)
    assert 'access_token' in response.json()

    data = jwt.decode(resp.json['access_token'], options={'verify_signature': False})
...
def test_logout_access(client):
    ...
    headers = {"Authorization": f"Bearer {auth_token}"}
    resp = client.post(f'/authorization/logout-access', headers=headers)

    assert response.json() == {"Access token has been revoked"}
    resp = client.post(f'/auth/logout-access', headers=headers)
